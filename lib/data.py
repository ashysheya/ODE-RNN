...
class DataGenerator(metaclass=abc.ABCMeta):
    ...
    def generate_task(self):
        ...
        if self.plot:
            x = np.linspace(*self.x_range, num=self.num_points_per_batch)
        ...
        for i in range(self.batch_size):
            ...
            if self.plot:
                ...
                start_point_index = np.random.randint(0, 
                    self.num_points_per_batch - num_points + 1)

            points_indicies = np.random.choice(np.arange(start_point_index + 1, 
                self.num_points_per_batch), size=num_points - 1, replace=False)
            points_indicies = np.concatenate([[start_point_index], 
                points_indicies, np.arange(self.num_points_per_batch, len(x))])
        ...
        task['extrap_time'] = self.extrapolation_range[0]
        task['extrap_index'] = (x[used_time_steps] <= self.extrapolation_range[0]).sum()
